...
test.describe("DELETE /BookStore/v1/Book", () => {
  test("API-COL-006 - should remove a book from the authenticated user collection", async ({
    ...
    try {
      ...
      const addBookResult = await addBookToUserCollection(
        request,
        testUser.userID,
        testUser.token,
        selectedBook.isbn,
      ...
      const { response: userBeforeRemovalResponse, body: userBeforeRemoval } =
        await getUserDetails(request, testUser.userID, testUser.token);

      expect(userBeforeRemovalResponse.status()).toBe(200);

      expect(userBeforeRemoval.books).toHaveLength(1);

      expect(userBeforeRemoval.books[0].isbn).toBe(selectedBook.isbn);
      ...
      const { response: userAfterRemovalResponse, body: userAfterRemoval } =
        await getUserDetails(request, testUser.userID, testUser.token);

      expect(userAfterRemovalResponse.status()).toBe(200);

      expect(userAfterRemoval.userId).toBe(testUser.userID);
      ...
      expect(
        userAfterRemoval.books.some((book) => book.isbn === selectedBook.isbn),
      ).toBe(false);
    } finally {
  ...
  test("API-COL-007 - should reject removing a book that is not in the user collection", async ({
    ...
    try {
      ...
      expect(removeBookResponse.headers()["content-type"]).toContain(
        "application/json",
      );

      const responseBody: unknown = await removeBookResponse.json();

      assertMatchesSchema<ApiErrorResponse>(
        apiErrorSchema,
        responseBody,
        "DELETE /BookStore/v1/Book book outside collection error",
      );

      expect(responseBody).toEqual(bookNotInUserCollectionError);
  ...
  test("API-COL-008 - should reject removing a book without an authentication token", async ({
    ...
    try {
      ...
      assertMatchesSchema<ApiErrorResponse>(
        apiErrorSchema,
        responseBody,
        "DELETE /BookStore/v1/Book missing token error",
      );
      ...
      expect(userAfterRemoval.books[0].isbn).toBe(selectedBook.isbn);
